...
def consistency_loss(logits_s, logits_w, class_acc, name='ce', p_cutoff=0.95, use_hard_labels=True):
    ...
    mask = F.greater_equal(max_probs, p_cutoff * (class_acc[max_idx] / (2. - class_acc[max_idx]))).astype('float32')
    select = F.greater_equal(max_probs, p_cutoff).astype('int32')
    if use_hard_labels:
        masked_loss = ce_loss(logits_s, max_idx, use_hard_labels, reduction='none') * mask
    else:
        print('must use hard label')
    return masked_loss.mean(), mask.mean(), select, max_idx, mask.astype('int32')
...
def nl_em_loss(pred_s, pred_w, k, mask_pred, p_cutoff):
    ...
    mask = F.where((mask==1)&(softmax_pred>p_cutoff**2), F.zeros_like(mask_k), mask)
